...
static WbMotionRef hand_wave, forwards, backwards, side_step_left, side_step_right, turn_left_60, turn_right_60, tai_chi,
  wipe_forehead, test, up, hand_wave_up, hand_wave_home, back_down;
...
//tìm và kích hoạt các cảm biến và cơ cấu điều khiển
static void find_and_enable_devices() {
  ...
  // ultrasound sensors: cảm biến siêu âm
...
static void load_motion_files() {
  ...
  up = wbu_motion_new("../../motions/Up.motion");
  hand_wave_up = wbu_motion_new("../../motions/HandWaveUp.motion");
  hand_wave_home = wbu_motion_new("../../motions/HandWaveHome.motion");
  back_down = wbu_motion_new("../../motions/BackDown.motion");
}
...
int main() {
  ...
  WbMotionRef motiontest[4];
  
  char *motionFiles[1] = {
    "../../motions/Dat/Test.motion"
  };
  
    for (int i = 0; i < 4; i++) {
      motiontest[i] = wbu_motion_new(motionFiles[i]);
    }
  
    // Phát chuyển động
    while(true){
      for (int i = 0; i < 4; i++) {
        wbu_motion_play(motiontest[i]);
    
        // Chờ cho đến khi chuyển động hoàn thành
        while (!wbu_motion_is_over(motiontest[i]))
    ...
    for (int i = 0; i < 4; i++) {
      wbu_motion_delete(motiontest[i]);
    }
